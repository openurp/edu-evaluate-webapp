  ...
  function Dates(){
       this.toDate=function(datestr){
         ...
         return new Date(Date.parse(datestr.substring(yearIdx+1,monthIdx) +"/"+datestr.substring(monthIdx+1)+"/"+datestr.substring(0,yearIdx)));
       };
	...
	function repeatChar(str,length){
   	...
   	/**
	 * 添加缩略表示 add a shortName to exists result; Do not use it directly. a
    ...
    function marshalByTeacherCourse(index){
    	...
    	if(this.activities[index].length==1){  
    	  ...
    	  return tname+" "+cname +delimiter+"(" +this.activities[index][0].marshal()+")";
    	}
    	else {
         	var marshalString="";
    	   ...
    	   // marshal tempActivities
    	    for(var m=0;m<tempActivities.length;m++){
                ...
                marshalString += delimiter+"(" +tempActivities[m].marshal()+")";	
	    	    for(var n=m+1;n<tempActivities.length;n++){
	                ...
	                if(tempActivities[n]!==null&&courseName==tempActivities[n].courseName&&teacherName==tempActivities[n].teacherName){	              
	              		marshalString += delimiter+"(" +tempActivities[n].marshal()+")";
    ...
    // return true,if this.activities[first] and this.activities[second] has same activities .
    ...
    this.teacherId=teacherId;
      ...
      return this;
   	...
   	function marshalTableForAdminclass(){
   		for(var k=0;k<this.unitCounts;k++){
   			if(this.activities[k].length>0){
   				this.marshalContents[k]= this.marshalForAdminclass(k);
   			}
   		}
 	}
   	
   	function marshalForAdminclass(index){
        if(this.activities[index].length===0){return "";}
    	if(this.activities[index].length==1){  
    	  var cname=this.activities[index][0].courseName;
    	  var tname=this.activities[index][0].teacherName;
    ...
    function CourseTable(beginOn,courseUnits){
        ...
        this.convertWeekstate2ReverseString=function (weekstate) {
        ...
        this.newActivity = function(teacherId,teacherName,courseId,courseName,roomId,roomName,startOn,weekstate,taskId,remark){
          var weekstate_str="";
          if(startOn){
             var weeks = Dates.weeksBetween(this.beginOn.getDay(),this.beginOn,Dates.toDate(startOn));
             if(weeks < 0){
               weekstate >>= (0 - weeks);
             }else{
               weekstate <<= weeks
           ...
           return new TaskActivity(teacherId,teacherName,courseId,courseName,roomId,roomName,weekstate_str,taskId,remark)
        }
       ...
       this.addActivityByTime = function(activity,weekday,beginAt,endAt){
         ...
         for (i =0 ; i< this.courseUnits.length;i++) {
           ...
           if (courseUnit[1] > beginAt && endAt > courseUnit[0]) {
             if ( (i+1) < startUnit)  startUnit = i+1;
             if ((i+1) > endUnit)   endUnit = i+1;
           }
         }
 ...
 this.detectCollisionInCell  = function (activitiesInCell) {
  ...
  return false;
	 ...
	 */ 
    function ActivityCluster(date,courseId,courseName,weeks,remark){
        this.courseId = courseId;
        this.courseName = courseName;
        this.weeks = weeks;
        this.remark = remark;
   ...
   */
    this.genActivities = function (){
        ...
        for(var teacherRoomId in this.activityMap){
           this.activityMap[teacherRoomId].vaildWeeks=this.buildWeekstate(teacherRoomId);
