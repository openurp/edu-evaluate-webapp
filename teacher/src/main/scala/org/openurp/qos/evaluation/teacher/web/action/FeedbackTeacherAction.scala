...
class FeedbackTeacherAction extends RestfulAction[Feedback] with ProjectSupport {
  ...
  def searchFeedback(): View = {
    ...
    // 获得(文字评教)
    var textEvaluations: Seq[Feedback] = Seq()
...
//    if (teacher == null) {
//      redirect("searchFeedback", "&clazz.id=" + clazzId
//        + "&semesterId=" + semesterId, "保存失败,你没有权限!")
//    }
//    // 创建对象
//    val std = entityDao.get(classOf[Student], stdId)
//    val textEvaluation = entityDao.get(classOf[Feedback], textEvaluationId)
...
//      redirect("searchFeedback", "&clazz.id=" + clazzId + "&semesterId=" + semesterId, "info.save.success")
//    } catch {
//      case e: Exception =>
//        e.printStackTrace()
//        redirect("searchFeedback", "&clazz.id=" + clazzId + "&semesterId=" + semesterId, "info.save.failure")
//    }
//  }
//
...
//    if (courseTakers.isEmpty) { redirect("searchFeedback", "保存失败,班级为空!", "&clazz.id=" + clazzId + "&semesterId=" + semesterId); }
//    // 创建对象
//    val textEvaluation = entityDao.get(classOf[Feedback], textEvaluationId)
//    val date = new java.util.Date()
//    val teacherRemessage = new TeacherRemessage()
