...
[/@]
    ...
    [@b.grid items=indicators var="indicator" sortable="false"]
        [@b.row]
            [@b.col title="问题类别" width="30%"]${(indicator.name)!}[/@]
            [@b.col title="总数" width="8%" ]
            [#assign nums = 0]
            [#list quesTypeNums as questionN]
            [#if questionN[0]?string ==  indicator.id?string]
            ...
            [#if questionDep[0]?string == indicator.id?string ]
            [#list quesTypeNums as questionN]
            [#if questionN[0]?string == indicator.id?string]
