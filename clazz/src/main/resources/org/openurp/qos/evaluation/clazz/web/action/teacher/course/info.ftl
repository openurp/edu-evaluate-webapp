...
<div style="width:800px;min-width:400px;margin:auto">
    <div class="header">
        <img class="header-photo" src="${base}/static/images/head-photo.jpg">
    ...
    <section class="view" style="background-color: rgb(249, 245, 244);">
        <div class="pj_container">
            ...
            [#list stat.indicators?sort_by(['indicator','code']) as indicatorDetail]
            <div class="title-1">
            ...
            <div class="fenxi">
                ...
                <span style="color: transparent;">空白</span>排名为：<b>${indicatorDetail.rank}/${categoryStat.courseCount}</b>
        ...
        <div class="pj_container">
            ...
            <div class="fenxi">
                ...
                课程在开课院系的排名：<b>${stat.rankInTeachDepart} / ${departmentStat.courseCount}</b><br>
                课程在所属${categoryStat.category.name}中的排名：<b>${stat.rankInCategory}  / ${categoryStat.courseCount}</b><br>
                与各档平均水平得分的对比如下，供参考。
            </div>
                  ...
                  var option = {
                      ...
                      series: [{
                         ...
                         [/#list]
                           {                    // 雷达图的数据是多变量（维度）的
                              name: '您的得分',                 // 数据项名称
                              value: [[#list stat.indicators?sort_by(['indicator','code']) as i]${i.score?string("#.00")}[#if i_has_next],[/#if][/#list]],        // 其中的value项数组是具体的数据，每个值跟 radar.indicator 一一对应。
                              symbol: 'circle',                   // 单个数据标记的图形。
                              symbolSize: 5,                      // 单个数据标记的大小，可以设置成诸如 10 这样单一的数字，也可以用数组分开表示宽和高，例如 [20, 10] 表示标记宽为20，高为10。
                              label: {                    // 单个拐点文本的样式设置
                                      normal: {
 ...
 <script>
        function initCharts_${stat.indicators?first.id}(echarts){
        [#list stat.indicators?sort_by(['indicator','code']) as i]
          init_chart_${i.id}(echarts);
        [/#list]
          init_radar_${stat.id}(echarts);
        }
        require(['echarts'],initCharts_${stat.indicators?first.id});
 </script>
[@b.foot/]
