...
[@b.form name="questionnaireStatTeachersForm" action="!search" target="contentDiv"]
    ...
    [@b.row]
        ...
        [#--[#list indicatorList?if_exists as indicator]
             [@b.col property="press.name" title="${indicator.name!}"  width="17%"]
             ${teacherStat.getTypeScoreDisplay(criteria,indicator.id)}
             [/@]
         [/#list]--]
        [#if teacherStat?exists]
            [#list teacherStat.indicatorStats as indicator]
