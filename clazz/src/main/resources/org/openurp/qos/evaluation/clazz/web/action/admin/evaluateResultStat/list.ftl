...
<@table.table id="listTable" width="100%" sortable="true">
   <@table.thead>
     ...
     <@table.sortTd width="7%" id="questionnaireStat.teacher.staff.code" text="教师工号"/>
    ...
    <@table.tbody datas=questionnaireStats;questionnaireStat>
        ...
        <td>${(questionnaireStat.teacher.staff.code)!}</td>
...
<script>
    ...
    var action="questionnaireStat.action";
      function exportData(){
       ...
       addInput(form,"keys","semesterStr,task.teachclass.name,task.course.name,teacher.staff.code,teacher.name,depart.name,questionnaire.description,validTickets,release,disPlayAvgScore,disPlayAddScore");
