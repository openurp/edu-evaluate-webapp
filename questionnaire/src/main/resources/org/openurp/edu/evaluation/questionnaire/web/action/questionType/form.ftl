...
[@b.form name="questionTypeForm" title="制定问题类别详细信息" action="!save" theme="list" ]
    ...
    [@b.datepicker label="生效日期" required="true" name="questionType.beginOn" id="_beginOn" format="yyyy-MM-dd" maxDate="#F{$dp.$D(\\'_endOn\\')}" value=(questionType.beginOn?string("yyyy-MM-dd"))! maxlength="10" style="width:200px"/]
    [@b.datepicker label="失效日期" name="questionType.endOn"  id="_endOn" format="yyyy-MM-dd" minDate="#F{$dp.$D(\\'_beginOn\\')}" value=(questionType.endOn?string("yyyy-MM-dd"))! maxlength="10" style="width:200px"/]
